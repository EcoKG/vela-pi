...
const MOCK_DEF: PipelineDef = {
  version: "1.2",
  pipelines: {} as PipelineDef["pipelines"],
  modes: {
    read: {
...
describe("VK-04: artifact_write_only", () => {
  ...
  it("allows Write inside .vela/artifacts/ in rw-artifact mode", () => {
    const r = checkToolCall(
      "Write",
      { file_path: "/project/.vela/artifacts/20240101T000000-slug/research.md" },
      "rw-artifact",
      null,
      undefined,
      MOCK_DEF
...
describe("Extensibility: custom blocked_tools are enforced", () => {
  ...
  it("allows a tool removed from modes.read.blocked_tools (custom permissive def)", () => {
    ...
    const permissiveDef: PipelineDef = {
      ...MOCK_DEF,
      modes: {
        ...MOCK_DEF.modes,
        read: {
...
describe("VK-05 / VK-06 security checks still active", () => {
  ...
  it("VK-06 blocks content with AWS key in write mode", () => {
    const r = checkToolCall(
      "Write",
      { file_path: "/project/config.ts", content: "const key = 'AKIAIOSFODNN7EXAMPLE1234'" },
      "write",
      null,
      undefined,
      MOCK_DEF
  ...
  it("VK-06 blocks Bash command containing Anthropic key", () => {
    const r = checkToolCall(
      "Bash",
      { command: "export KEY=sk-ant-api03-aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa" },
      "readwrite",
      null,
      undefined,
      MOCK_DEF
