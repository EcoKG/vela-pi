...
import { createAgentSession, readOnlyTools, codingTools, SessionManager, } from "@mariozechner/pi-coding-agent";
const ROLE_CONFIGS = {
    researcher: {
 ...
 */
export async function runVelaAgent(opts) {
    ...
    const tools = roleConfig.toolSet === "readOnly"
        ? readOnlyTools
        : codingTools;
...
async function runParallelResearch(opts) {
    ...
    const perspectiveResults = await Promise.allSettled(perspectives.map(async ({ key, focus }) => {
        const sessionOpts = {
            cwd,
            tools: readOnlyTools,
            sessionManager: SessionManager.inMemory(cwd),
...
Analyse the codebase and write your findings as markdown. Be concise (200-400 tokens).`;
    ...
    const sections = [`# Research: ${request}\n`];
...
function accumulateLearning(cwd, request, _artifactDir, learningText) {
    try {
...
function buildContextPrompt(artifactDir, role) {
    const inputFiles = {
        researcher: [],
        planner: ["research.md"],
...
function buildTaskPrompt(role, request, taskType, contextFiles) {
    const lines = [
        `## Task`,
        `Request: ${request}`,
        `Type: ${taskType}`,
