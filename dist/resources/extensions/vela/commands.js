...
async function cmdTransition(ctx) {
    ...
    if (!result.ok) {
        ...
        ctx.ui.notify(`[Vela] Cannot transition: ${result.error}\n` +
            (missingList ? `  Missing:\n    ${missingList}` : ""), "warning");
        return;
    }
    if (result.completed) {
    ...
    ctx.ui.notify(`[Vela] Advanced: ${result.previous_step} → ${result.current_step}\n` +
        `  Step: ${result.current_step_name}\n` +
        `  Mode: ${result.current_mode}`, "info");
}
async function cmdRecord(parts, ctx) {
    ...
    const autoNote = result.auto_disabled
        ...
        : "";
    ctx.ui.notify(`[Vela] Recorded ${result.verdict} for step "${result.step}" (revision ${result.revision}).${autoNote}`, "info");
}
...
async function cmdHistory(ctx) {
    ...
    for (const p of pipelines.slice(0, 20)) {
        ...
        lines.push(`  ${icon} ${p.date} ${p.slug.split("-").slice(2).join("-").substring(0, 25).padEnd(25)} ` +
...
async function cmdSprintRun(request, cwd, ctx) {
    ...
    ctx.ui.notify(`[Vela] Sprint created: "${plan.title}"\n` +
        `  ID: ${plan.id}\n` +
...
async function executeSprintSlices(sprintId, cwd, ctx) {
    ...
    while (true) {
        ...
        if (next.action === "run" && next.slice) {
            ...
            ctx.ui.notify(`[Vela] Executing slice ${iteration}/${plan.total_slices}: ${slice.title}`, "info");
...
async function cmdDispatch(parts, ctx) {
    ...
    ctx.ui.notify(`[Vela] Dispatching agent: role=${role}, mode=${mode}\n  This may take a few minutes...`, "info");
    const result = await runVelaAgent({
        role,
        cwd,
        artifactDir,
    ...
    if (!result.ok) {
        ctx.ui.notify(`[Vela] Agent dispatch failed (${role}): ${result.error}`, "warning");
        return;
    }
    const duration = result.durationMs ? `${Math.round(result.durationMs / 1000)}s` : "";
    ctx.ui.notify(`[Vela] Agent completed: ${role}\n` +
        `  Artifact: ${result.artifact ?? "—"}\n` +
        (duration ? `  Duration: ${duration}\n` : "") +
        "\nRun /vela transition when ready to advance.", "info");
}
...
async function runAutoLoop(ctx) {
    ...
    while (maxIterations-- > 0) {
        ...
        if (currentStep.actor === "user" || currentStep.actor === "pm") {
            ctx.ui.notify(`[Vela] Auto paused at "${state.current_step}" (${currentStep.actor} step).\n` +
                `  Complete manually then run /vela transition`, "info");
            break;
        }
        ...
        if (!result.ok) {
            ctx.ui.notify(`[Vela] Auto: transition blocked at "${freshState2.current_step}"\n  ${result.error}\n  Missing: ${result.missing?.join(", ")}`, "warning");
            break;
        }
...
async function cmdCancel(ctx) {
    ...
    if (state.git?.is_repo) {
        if (state.git.pipeline_branch) {
            hints.push(`To discard pipeline branch: git checkout ${state.git.base_branch} && git branch -d ${state.git.pipeline_branch}`);
        }
        else if (state.git.checkpoint_hash) {
            hints.push(`To see pipeline changes: git diff ${state.git.checkpoint_hash}..HEAD`);
        }
    }
    ctx.ui.notify(`[Vela] Pipeline cancelled at step "${state.current_step}".\n` +
