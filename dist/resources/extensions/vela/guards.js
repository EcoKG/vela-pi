...
function checkGateGuard(toolName, toolInput, state, cwd) {
    ...
    if (WRITE_TOOLS.has(toolName)) {
        ...
        if (PRE_EXECUTE_STEPS.has(currentStep) && filePath) {
            ...
            const isVelaWrite = normalized.includes("/.vela/artifacts/") ||
        ...
        if (filePath && (filePath.endsWith("report.md") || filePath.endsWith("/report.md"))) {
            const artifactDir = state._artifactDir ?? state.artifact_dir;
            if (artifactDir) {
                const verifyExists = existsSync(join(artifactDir, "verification.md")) ||
        ...
        if (currentStep === "execute" && cwd && filePath && !filePath.includes("/.vela/")) {
            ...
            // Validate schema
            try {
                const del = JSON.parse(readFileSync(delegationPath, "utf8"));
