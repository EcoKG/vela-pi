 ...
 */
export function findActivePipelineState(cwd) {
    try {
...
// ─── Pipeline Definition ──────────────────────────────────────────────────────
export function loadPipelineDefinition(cwd, extensionDir) {
    const candidates = [
...
// ─── Step Resolution ──────────────────────────────────────────────────────────
export function resolveSteps(def, pipelineType) {
...
export function getCurrentStep(state, def) {
    const steps = resolveSteps(def, state.pipeline_type);
    return steps.find((s) => s.id === state.current_step) ?? null;
}
export function getCurrentMode(state, def) {
    if (!state || !def)
        return "readwrite";
    const step = getCurrentStep(state, def);
    return step?.mode ?? "readwrite";
}
// ─── Exit Gate ────────────────────────────────────────────────────────────────
export function checkExitGate(stepDef, state) {
    ...
    for (const gate of stepDef.exit_gate) {
        switch (gate) {
            ...
            case "git_clean":
                ...
                break;
            case "init_complete":
                if (!completedSteps.includes("init"))
                    missing.push(gate);
                break;
            ...
            case "plan_check_pass":
                ...
                break;
            case "user_approved":
                if (state.current_step === "checkpoint" &&
            ...
            case "implementation_complete": {
                if (artifactDir) {
                    ...
                    if (!existsSync(execApproval)) {
                        ...
                        try {
                            const a = JSON.parse(readFileSync(execApproval, "utf8"));
            ...
            case "branch_created":
                if (state.git?.is_repo) {
                    if (!state.git.pipeline_branch && state.current_step === "branch") {
                        if (!revisions["branch"] || revisions["branch"] < 1)
                            missing.push(gate);
                    }
                }
                break;
            case "changes_committed":
                if (state.git?.is_repo) {
                    if (!state.git.commit_hash && state.current_step === "commit") {
                        if (!revisions["commit"] || revisions["commit"] < 1)
                            missing.push(gate);
                    }
                }
                break;
            ...
            case "ref_integrity": {
                ...
                if (!existsSync(diffPath))
                    break; // non-fatal if no diff yet
                try {
...
export function transitionPipeline(state, def) {
    const steps = resolveSteps(def, state.pipeline_type);
...
export function recordStep(state, verdict, summary) {
    ...
    const autoDisabled = state.auto === false && (state.auto_reject_count ?? 0) >= 2 && (v === "reject" || v === "fail");
    return {
        ok: true,
        step: state.current_step,
        verdict: v,
...
export function subTransitionPipeline(state) {
...
// ─── Git Helpers ──────────────────────────────────────────────────────────────
export function snapshotGitState(cwd) {
    try {
...
export function createPipelineBranch(cwd, state, mode = "auto") {
    if (!state.git?.is_repo) {
    ...
    try {
        ...
        try {
            ...
            return { ok: false, action: "error", error: e2.message };
        }
    }
...
export function commitPipeline(cwd, state, def, messageOverride) {
    if (!state.git?.is_repo) {
    ...
    if (!statusOut) {
        ...
        if (state.git)
            state.git.commit_hash = hash;
        state.updated_at = new Date().toISOString();
        persistState(state);
        return { ok: true, action: "no_changes", hash };
    }
    const typeMap = (def?.git?.commit?.type_map) ?? { code: "feat", "code-bug": "fix", "code-refactor": "refactor", docs: "docs", infra: "chore" };
    ...
    try {
        ...
        for (const vf of velaInternals) {
            try {
                gitExec(cwd, "reset", "HEAD", "--", vf);
            }
            catch { /* skip */ }
        }
    }
    catch (e) {
        return { ok: false, error: `Failed to stage: ${e.message}` };
    }
    try {
...
export function listPipelineHistory(cwd) {
    const artifactsDir = join(cwd, ".vela", "artifacts");
    if (!existsSync(artifactsDir))
        return [];
    const result = [];
...
export function cleanupCancelledArtifacts(cwd, hoursOld = 24) {
    const artifactsDir = join(cwd, ".vela", "artifacts");
    if (!existsSync(artifactsDir))
        return 0;
        ...
        for (const dir of readdirSync(artifactsDir).filter((d) => /^\d{8}T\d{6}-/.test(d))) {
            ...
            try {
                ...
                if (state.status !== "cancelled" && state.status !== "completed")
                    continue;
                if (statSync(statePath).mtimeMs > cutoff)
                    continue;
 ...
 */
export function cleanupStalePipelines(cwd, hoursOld = 48) {
    const artifactsDir = join(cwd, ".vela", "artifacts");
    if (!existsSync(artifactsDir))
        return 0;
    let count = 0;
...
function appendTrace(artifactDir, entry) {
    try {
